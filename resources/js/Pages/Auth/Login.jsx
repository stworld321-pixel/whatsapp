...
function firebaseErrorMessage(error, fallback) {
    const messages = {
        ...
        'auth/invalid-auth-domain': 'The Firebase authentication domain is invalid.',
        'auth/popup-blocked': 'The sign-in popup was blocked by the browser.',
        'auth/network-request-failed': 'Firebase could not be reached. Check your network connection.',
    };
...
function FirebaseGoogleButton() {
    ...
    useEffect(() => {
        if (initRef.current || !enabled || !apiKey) return;
        ...
        import('@/lib/firebase')
            ...
            .catch((err) => {
                setError(firebaseErrorMessage(err, t('auth.google_signin_failed')));
            });
    }, [enabled, apiKey, authDomain, projectId, appId, t]);
