...
{
    /**
     * Get the appropriate timestamp diff expression for the current database driver.
     * MySQL uses TIMESTAMPDIFF, SQLite uses julianday conversion.
     */
    private function timestampDiff(string $unit, string $start, string $end): string
    {
        if (DB::connection()->getDriverName() === 'sqlite') {
            // SQLite: convert to Julian day and multiply
            return match ($unit) {
                'SECOND' => "(julianday({$end}) - julianday({$start})) * 86400",
                'MINUTE' => "(julianday({$end}) - julianday({$start})) * 1440",
                'HOUR'   => "(julianday({$end}) - julianday({$start})) * 24",
    ...
    /**
     * Get the appropriate date format expression for the current database driver.
     * MySQL uses DATE_FORMAT, SQLite uses strftime.
     */
    private function dateFormat(string $field, string $format): string
    {
        if (DB::connection()->getDriverName() === 'sqlite') {
    ...
    {
        $hourFormat = $this->dateFormat('sent_at', '%Y-%m-%d %H:00');
        $sent = CampaignRecipient::where('campaign_id', $campaignId)
            ->whereNotNull('sent_at')
            ->selectRaw("{$hourFormat} as hour, COUNT(*) as total")
            ->groupBy('hour')->orderBy('hour')
            ->pluck('total', 'hour')->toArray();

        $hourFormat = $this->dateFormat('delivered_at', '%Y-%m-%d %H:00');
        ...
        $hourFormat = $this->dateFormat('read_at', '%Y-%m-%d %H:00');
        ...
        $row = CampaignRecipient::where('campaign_id', $campaignId)
            ->selectRaw('
                AVG(CASE WHEN sent_at IS NOT NULL AND delivered_at IS NOT NULL
                    THEN ' . $this->timestampDiff('SECOND', 'sent_at', 'delivered_at') . ' END) as sent_to_delivered,
                AVG(CASE WHEN delivered_at IS NOT NULL AND read_at IS NOT NULL
                    THEN ' . $this->timestampDiff('SECOND', 'delivered_at', 'read_at') . ' END) as delivered_to_read,
                AVG(CASE WHEN sent_at IS NOT NULL AND read_at IS NOT NULL
                    THEN ' . $this->timestampDiff('SECOND', 'sent_at', 'read_at') . ' END) as sent_to_read
            ')
            ->first();
    ...
    public function agentLeaderboard(int $wsId, Carbon $from, Carbon $to): array
    {
        ...
        $tsDiff = $this->timestampDiff('MINUTE', 'first_in.created_at', 'first_out.sent_at');
        $firstResponse = DB::select("
            SELECT
                c.assigned_user_id,
                AVG({$tsDiff}) AS avg_first_response_min
    ...
    {
        $weekFormat = $this->dateFormat('created_at', '%Y-%u');
        $rows = Client::selectRaw("{$weekFormat} as week, COUNT(*) as clients")
