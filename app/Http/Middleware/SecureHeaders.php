...
class SecureHeaders
{
    ...
    private function buildCsp(): ?string
    {
        ...
        $frameSrc = "'self'".$this->authenticationFrameSources();
    ...
    /** OneSignal (when configured), Firebase Authentication, Meta JS SDK, and Cloudflare Web Analytics / beacon scripts. */
    ...
    private function connectSources(): string
    {
        ...
        // Firebase Auth exchanges credentials and refreshes tokens with these APIs.
        $sources[] = 'https://identitytoolkit.googleapis.com';
    ...
    /** Allow Firebase and Meta authentication dialogs in iframes. */
    private function authenticationFrameSources(): string
    {
        // Firebase Auth uses an iframe on the configured firebaseapp.com domain
        // and opens Google identity pages in accounts.google.com.
        $sources = ' https://*.firebaseapp.com https://*.web.app https://accounts.google.com';

        if (! $this->metaSdkEnabled()) {
            return $sources;
        }

        return $sources.' https://www.facebook.com https://web.facebook.com https://connect.facebook.net';
    }
