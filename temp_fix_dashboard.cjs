...
const trackerBlock = `function TrackerBar({ label, current, limit, percent, icon: Icon, accent = 'bg-brand-500' }) {
...
s = s.replace(`<TrackerBar {...trackerRow} />`, `{trackerRows.map((row) => {
...
const oldVars = `const currentUsage = usePage().props.current_workspace_usage ?? {};
...
const newVars = `const currentUsage = usePage().props.current_workspace_usage ?? {};
